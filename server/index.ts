...
async function scanFiles (scanPath: string, relPath: string = ''): Promise<Array<FileEntry | FolderEntry>> {
  const files: Array<FileEntry | FolderEntry> = []
...
app.get('/files/*', asyncHandler(async (req, res): Promise<void> => {
  if (req.path.includes('/.')) {
    res.status(401).send('can\'t edit hidden files')
    return
  }
  const {
    create: createPath,
  ...
  if (createPath !== undefined && createPath !== '') {
  ...
  if (!exists) {
    ...
    if (isDir) {
      ...
      res.status(404).send('file doesn\'t exist and that extension isn\'t allowed')
      return
    }
  }
  if (isDir) {
    ...
    files.sort((a, b) => a.isDir === b.isDir
      ? +b.isDir - +a.isDir
      : a.name.localeCompare(b.name))
    if (asComponent !== undefined && asComponent !== '') {
      res.render('directory', {
        files,
        baseUrl,
        sidebar: sidebarMode !== undefined && sidebarMode !== '',
      })
    } else {
    ...
    if (sendFile !== undefined && sendFile !== '') {
...
wsApp.ws('/wuss', async (ws, req) => {
  const info = {
    name: req.session?.name === '' ? (req.session?.name as string) : 'Suspicious unknown user from ' + req.ip,
    start: new Date(),
  }
  ...
  async function onMessage (msg: WebSocket.Data): Promise<void> {
...
app.listen(port, () => {
  ...
  console.log('')
  console.log(`Anyone who knows the password as set in your .env file can manipulate the files at ${baseDir} that satisfy ${safeExtensions.toString()}`)
  console.log('')
